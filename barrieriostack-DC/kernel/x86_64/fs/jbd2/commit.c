...
int lwj_schedule_time = 0;
struct timeval commit_start_time;
struct timeval commit_end_time = {0, };
...
/* UFS: The last block of journal metadata  */
static void journal_end_logbuf_io_async(struct buffer_head *bh, int uptodate)
{
	struct buffer_head *orig_bh = bh->b_private;
 ...
 * It wakes up pending threads for BH_Shadow
 */
...
void jbd2_journal_barrier_commit_transaction(journal_t *journal)
{
	...
	J_ASSERT(commit_transaction->t_state == T_RUNNING);
        ...
        if(lwj_commit_time != 0){
                printk("[lwj_check_time](pid:%-4u)Delay Start!!\n", current->pid);
                do_gettimeofday(&commit_start_time);
                while((commit_start_time.tv_sec*1000000+commit_start_time.tv_usec) - (commit_end_time.tv_sec*1000000+commit_end_time.tv_usec) < lwj_commit_time){
                        //struct timeval before_schedule;
                        //do_gettimeofday(&before_schedule);
                        schedule();
                        do_gettimeofday(&commit_start_time);
/*
                        if(atomic_read(&commit_transaction->t_updates)==0)
                                break;

                        if((commit_start_time.tv_sec*1000000+commit_start_time.tv_usec) - (before_schedule.tv_sec*1000000+before_schedule.tv_usec) < lwj_schedule_time)
                                break;
*/
                }
        }
        printk("[lwj_check_time](pid:%-4u)Delay End!!\n", current->pid);
...
start_journal_io:
	...
	*/
	 ...
	 * removed. It will be performed in cpthread. We insert this transaction
	...
	wake_up(&journal->j_wait_cpsetup);
        ...
        /* lwj delayed commit */
        do_gettimeofday(&commit_end_time);
}
